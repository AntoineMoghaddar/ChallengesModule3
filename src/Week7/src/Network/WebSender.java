...
public class WebSender implements Runnable {
	...
	public WebSender(String serverIP, LinkedBlockingQueue<Position> data, String name) {
		this.name = name;
		this.data = data;
		try {
	...
	public void run() {
		...
		DatagramPacket packet = new DatagramPacket(buf, 1024, serverAddress, 4742);
		...
		try {
			...
			while (true) {
				...
				ByteBuffer data = ByteBuffer.allocate(3 + nameArray.length + xArray.length + yArray.length);
